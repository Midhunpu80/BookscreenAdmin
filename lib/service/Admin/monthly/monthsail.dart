...
class month_service extends GetxController {
  var isLoading = false.obs;

  late Montsail10 reply;
  getmonthsail() async {
    try {
      isLoading(true);
      end j = end();
      final response = await http.get(Uri.parse(j.monthy_url), headers: {
      ...
      if (response.statusCode == 200) {
        reply = montsail10FromJson(response.body.toString());
